...
void Linker::link(const std::string& objFile1, const std::string& objFile2, const std::string& outputFile) {
    ...
    // Build global definition table with correction factors
    for (auto& [symbol, address] : globalSymbolTable) {
        for (auto& [usageSymbol, usagePos] : usageTable1) {
            if (symbol == usageSymbol) {
                address += correctionFactor2;
            }
        }
    }
