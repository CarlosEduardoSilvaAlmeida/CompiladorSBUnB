    ...
    while (iss.get(ch))
    {
        if (ch == ' ' || ch == '\t' || ch == ',' || ch == '+' || ch == '-')
        {
            if (!token.empty())
            {
                ...
                if(ch == '+' || ch == '-'){
                    tokens.push_back(std::string(1, std::toupper(static_cast<unsigned char>(ch))));
                }
                token.clear();
            }
        }
        ...
        {
            token.push_back(std::toupper(static_cast<unsigned char>(ch)));
        }
    }
...
bool Assembler::isValidOpcode(const std::string &opcode)
{
    static const std::unordered_set<std::string> validOpcodes = {
        "ADD", "SUB", "MULT","MUL", "DIV", "JMP", "JMPN", "JMPP", "JMPZ", "COPY",
...
bool Assembler::hasCorrectNumberOfOperands(const std::string &opcode, size_t numOperands)
{
    static const std::unordered_map<std::string, size_t> opcodeOperands = {
        {"ADD", 1}, {"SUB", 1}, {"MULT", 1}, {"MUL", 1}, {"DIV", 1}, {"JMP", 1}, {"JMPN", 1}, {"JMPP", 1}, {"JMPZ", 1}, {"COPY", 2}, {"LOAD", 1}, {"STORE", 1}, {"INPUT", 1}, {"OUTPUT", 1}, {"STOP", 0}, {"SPACE", 0}, {"CONST", 1}, {"BEGIN", 0}, {"END", 0}};
...
int Assembler::getOpcodeValue(const std::string &opcode)
{
    static const std::unordered_map<std::string, int> opcodeValues = {
        {"ADD", 1}, {"SUB", 2}, {"MULT", 3}, {"MUL", 3}, {"DIV", 4}, {"JMP", 5}, {"JMPN", 6}, {"JMPP", 7}, {"JMPZ", 8}, {"COPY", 9}, {"LOAD", 10}, {"STORE", 11}, {"INPUT", 12}, {"OUTPUT", 13}, {"STOP", 14}};
...
void Assembler::assemble(const std::string &inputFile, const std::string &finalOutputFile)
{
    ...
    while (std::getline(input, line))
    {
        ...
        if (!label.empty())
        {
            ...
            {
                std::cout << "Processing CONST directive." << operands[0] <<std::endl;
            ...
            else
            {
                ...
                if(symbolTable.find(label) != symbolTable.end()){
                    if(symbolTable[label].isExtern)
                        throw std::runtime_error("Error: Symbol extern: " + label);
                }

                
                definitionTable[label] = locationCounter;
                symbolTable[label].address = locationCounter; // Endereço atual e não externo
                symbolTable[label].isResolved = true;
                symbolTable[label].isExtern = false;
                
                cout << "Label: " << label << " Address: " << locationCounter << endl;
                if (isValidOpcode(opcode))
                    {
                        tempOutput << getOpcodeValue(opcode) << " ";
                        real.push_back("0");
                        locationCounter++;
                    }
                ...
                if (!std::regex_match(operandos, std::regex("^\\d+$")))
                    {
                        if (std::regex_match(operandos, std::regex("^[A-Za-z]+[+-]\\d+$")))
                        {
                            // Confere se operando é uma operação entre uma variável e um número
                            std::smatch match;
                            std::regex_search(operandos, match, std::regex("([A-Za-z]+)([+-])(\\d+)"));
                            std::string var = match[1];
                            char op = match[2].str()[0];
                            int num = std::stoi(match[3]);
            
                            if ((symbolTable.find(var) != symbolTable.end())
                                || (definitionTable.find(var) != definitionTable.end() && definitionTable[var] != -1))
                            {
                                if(symbolTable[var].isExtern)
                                {
                                    usageTable[var].push_back(locationCounter);
                                }
                                int varAddress = symbolTable[var].address;
                                int result = (op == '+') ? varAddress + num : varAddress - num;
                                real.push_back("1");
                                tempOutput << result << " ";
                                locationCounter++;
                            }
                            
                        } else if ((symbolTable.find(operandos) != symbolTable.end()) && symbolTable[operandos].isExtern)
                        {
                            
                            usageTable[operandos].push_back(locationCounter);
                            locationCounter++;
                        }
                        else if (!symbolTable[operandos].isResolved)
                        {
                            pendingReferences[operandos].push_back(locationCounter);
                            locationCounter++;
                            
                        } else if (definitionTable.find(operandos) != definitionTable.end())
                        {
                            cout << "Operand: " << operandos << " Address: " << definitionTable[operandos] << endl;
                            tempOutput << definitionTable[operandos] << " ";
                            real.push_back("1");
                            locationCounter++;
                        }
                    }
                    else if (isValidImmediateValue(operandos))
                    {
                        tempOutput << operandos << " ";
                        real.push_back("1");
                        locationCounter++;
                    }
                    else
                    {
                        throw std::runtime_error("Error: Invalid operand: " + operandos);
                    }
            }
        }
        else if (opcode == "END")
